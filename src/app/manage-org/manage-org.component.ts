...
import { DeactivateOrgComponent } from "./deactivate-org/deactivate-org.component";
...
@Component({
  ...
  styleUrls: ["./manage-org.component.less"],
})
export class ManageOrgComponent implements OnInit {
  ...
  getAllOrgdetails() {
    ...
    this.orgDetails = [];
    this.manageOrgService.getActiveOrgDetails().subscribe(
      (data) => {
        if (data.map.statusMessage == "Success") {
          ...
          for (var i = 0; i < this.orgDetails.length; i++) {
            this.orgDetails[i].expiry_date = new Date(this.orgDetails[i].expiry_date);
            this.orgDetails[i].date_of_joining = new Date(this.orgDetails[i].date_of_joining);
            if (this.orgDetails[i].pricingPlanDetails) {
              let word = this.orgDetails[i].pricingPlanDetails.currency;
              this.orgDetails[i].pricingPlanDetails.currency = word
                .split(" ")
                .pop();
              this.orgDetails[i].pricingPlanDetails.modules = JSON.parse(
                this.orgDetails[i].pricingPlanDetails.modules
              );
            }
          }
          if (this.orgDetails.length) {
            this.orgDataSource = new MatTableDataSource(
              this.orgDetails.reverse()
            );
  ...
  deactivateOrgdetails(id: any, comment: any) {
    ...
    this.manageOrgService.deactivateOrg(data).subscribe(
      (data) => {
        if (data.map.statusMessage == "Success") {
          this.utilsService.openSnackBarAC(
            "Organization details deactivated successfully",
            "OK"
          );
          this.ngOnInit();
          this.spinner.hide();
          ...
          this.utilsService.openSnackBarMC(
            "Failed to deactivate organization details",
            "OK"
          );
          this.spinner.hide();
        }
  ...
  bulkDeleteDetails() {
    ...
    this.manageOrgService.bulkdeleteOrg(listdata).subscribe(
      (data) => {
        if (data.map.statusMessage == "Success") {
          this.utilsService.openSnackBarAC(
            "Organization details deleted successfully",
  ...
  bulkdeactivateDetails(comment: any) {
    ...
    this.manageOrgService.bulkDeactivateOrg(listdata).subscribe(
      (data) => {
        if (data.map.statusMessage == "Success") {
          this.utilsService.openSnackBarAC(
            "Organization details deactivated successfully",
          ...
          this.utilsService.openSnackBarMC(
            "Faild to deactivate organization details",
            "OK"
          );
        }
        this.spinner.hide();
  ...
  getallpendingdetails() {
    this.spinner.show();
    this.pending_req = 0;
    this.manageOrgService.getAllOrgDetails().subscribe(
      (data) => {
        if (data.map.statusMessage == "Success") {
          let response: any[] = JSON.parse(data.map.data);
          for (var i = 0; i < response.length; i++) {
  ...
  renewplan(id, plan, req, plan_id, modules) {
    ...
    const dialogRef = this.dialog.open(BulkActDeactComponent, {
      ...
      data: {
        ...
        req: req,
        plan_id: plan_id,
        modules: this.modified_plan_module,
      },
    });
    dialogRef.afterClosed().subscribe((resp) => {
      this.selection.clear();
